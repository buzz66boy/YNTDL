...
void yntdl::validateNode(std::shared_ptr<yntdl::Node> nodePtr){
    bool fail = false;
    if(nodePtr->name == ""){
    ...
    for(auto app : nodePtr->applications){
        if(app.name == ""){
            ...
            throw yntdl::YntdlException(yntdl::ErrorCode::APP_INVALID, name);
        }
    }

    if(fail){
        throw yntdl::YntdlException(yntdl::ErrorCode::NODE_INVALID, name);
    }
}
...
int Settings::parse_settings_file(std::string settings_file){
	...
	if(!settings){
		...
		throw yntdl::YntdlException(yntdl::ErrorCode::FILE_NOT_FOUND, settings_file);
	}

	if(settings[NS3_PATH_STR]){
		Settings::ns3_path = settings[NS3_PATH_STR].as<std::string>();
	} else {
		throw yntdl::YntdlException(yntdl::ErrorCode::SETTINGS_ATTR_NOT_FOUND, NS3_PATH_STR);
	}

	// check for required destinations and error if not found
	struct stat buffer;
	if(stat(Settings::ns3_path.c_str(), &buffer) != 0 || !S_ISDIR(buffer.st_mode)){
		throw yntdl::YntdlException(yntdl::ErrorCode::FOLDER_NOT_FOUND, Settings::ns3_path);
	}

	if(settings[TEMP_DEST_STR]){
		Settings::temp_dir = settings[TEMP_DEST_STR].as<string>();
	} else {
		throw yntdl::YntdlException(yntdl::ErrorCode::SETTINGS_ATTR_NOT_FOUND, TEMP_DEST_STR);
	}

	if(!check_make_dir(Settings::temp_dir.c_str())){
		throw yntdl::YntdlException(yntdl::ErrorCode::FOLDER_NOT_CREATED, Settings::temp_dir);
	}
	...
	if(!check_make_dir(Settings::script_dest.c_str())){
		throw yntdl::YntdlException(yntdl::ErrorCode::FOLDER_NOT_CREATED, Settings::script_dest);
	}
	...
	if(!check_make_dir(Settings::output_dest.c_str())){
		throw yntdl::YntdlException(yntdl::ErrorCode::FOLDER_NOT_CREATED, Settings::output_dest);
	}

	Settings::container_config_dir = Settings::temp_dir + CONTAINER_CONFIG;
	if(!check_make_dir(Settings::container_config_dir.c_str())){
		throw yntdl::YntdlException(yntdl::ErrorCode::FOLDER_NOT_CREATED, Settings::container_config_dir);
	}
